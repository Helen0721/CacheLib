...
def plot(all_res,
    ...
    final_hrs = [sum(hr_list)/len(hr_list) for hr_list in hr_lists]
    if ap.limit:
    ...
    fig, axs = plt.subplots(nrows=len(thpt_labels), ncols=len(thpt_labels[0]), figsize=(8,5))
    ...
    formatter.set_powerlimits((-3, 3))  # Adjust these limits to control when scientific notation is used
    ...
    for r in range(len(thpt_labels)):
        for i in range(len(thpt_labels[0])):
            ...
            if r == 0: axs[r,i].xaxis.set_major_formatter(formatter)
