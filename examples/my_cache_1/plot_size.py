...
def plot_mr_size(cache_sizes,hr_lists,labels,plot_folder,plot_name,plot_title,suffix=""): 
    ...
    legend = plt.legend(ncol= (num_lines // 4 if num_lines > 3 else num_lines ), 
                        loc="upper right", fontsize="10" if num_lines < 3 else "6", frameon=False) 
...
def plot_bar(cache_sizes,hr_lists,labels,plot_folder,plot_name,plot_title,suffix=""): 
    num_lines = len(hr_lists) 
    if num_lines==0: 
        print("no plot to plot.")  
        return
    ...
    plot_fname = os.path.join(plot_folder,"mrp-size-{}-bar".format(plot_name))
    ...
    bar_height = 0.5 / n_bars 
    bar_positions = np.arange(len(cache_sizes))
    
    fig, ax = plt.subplots(figsize=(10, 6))
    
    for i in range(n_bars):
        # Calculate the position for each bar within the group
        bar_pos = bar_positions + i * bar_height 
        data = [1-float(hr) for hr in hr_lists[i]]
        ax.barh(bar_pos, data, bar_height, label=labels[i])
    ...
    ax.margins(y=0.01)
    plt.ylabel('Cache Sizes',fontsize=20)
    plt.yticks(bar_positions, cache_sizes, fontsize=20)
    
    legend = plt.legend( 
            fontsize="20", frameon=False,borderaxespad=0.,
    ...
    pp.close()
    print("bar plot saved to {}".format(plot_fname))
...
def handle_mr():
        ...
        for (j,rebalance_strategy) in enumerate(rebalance_strategies):
            ...
            for cache_size in cache_sizes:
                ...
                if json_RES:
                    ...
                    if rebalance_strategy == "noReb":
                        output_file = os.path.join(ap.output_folder,rebalance_strategy,"{}_{}_{}".format(ap.name,algo,cache_size))
                    else:
    ...
    if "bar" in ap.type:
        plot_bar(cache_sizes,hr_lists,labels,plot_folder=ap.plot_folder,plot_name=name,plot_title=title)
    else:
...
def handle_stopReb():
    ...
    for (i,algo) in enumerate(algos):
        for (j,reb) in enumerate(rebalance_strategies):
            ...
            for cache_size in cache_sizes:
                if reb == "noReb":
                    output_file = os.path.join(ap.output_folder,reb,"{}_{}_{}".format(ap.name,algo,cache_size))
                else:
                    output_file = os.path.join(ap.output_folder,reb,
