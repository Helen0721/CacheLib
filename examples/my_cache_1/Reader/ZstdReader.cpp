...
void WARN(std::string msg, const char *err){
	std::cout << msg << err;
}
...
size_t zstd_reader_read_line(zstd_reader *reader, char **line_start,
  ...
  if (reader->buff_out_read_pos < reader->output.pos) {
    ...
    *line_start = (char *)reader->buff_out + reader->buff_out_read_pos;
    char *buff_start = (char *) reader->buff_out + reader->buff_out_read_pos;
    size_t buff_left_sz = reader->output.pos - reader->buff_out_read_pos;
...
size_t zstd_reader_read_bytes(zstd_reader *reader, size_t n_byte,
  ...
  if (reader->buff_out_read_pos + n_byte <= reader->output.pos) {
    ...
    *data_start = ((char *)reader->buff_out) + reader->buff_out_read_pos;
    ...
    std::cout << (reader->output.pos - reader->buff_out_read_pos) << " < " << n_byte;
    ERROR("do not have enough bytes"); 
...
void print_one_zstd_request(zstd_request *obj){
  //std::cout<<"clock_time:"<<obj->clock_time<<"id:"<<obj->obj_id<<"size:"<<obj->obj_size<<"op:"<<obj->op<<"ns:"<<obj->ns<<"nt:"<<obj->next_access_vtime<<std::endl;
   //std::cout<<"clock_time:"<<obj->clock_time<<"id:"<<obj->obj_id<<"size:"<<obj->obj_size<<"nt:"<<obj->next_access_vtime<<std::endl;
   int64_t t = (obj->next_access_vtime == -1 ) ? INT64_MAX : obj->next_access_vtime;
   std::cout<<obj->clock_time<<","<<obj->obj_id<<","<<obj->obj_size<<","<<t<<std::endl;
}
