...
def plot_hr_time(ts_lists,hr_lists,labels,name=""): 
    ...
    plt.savefig("{}/hrp-{}.pdf".format(PLOTDIR,name), bbox_inches="tight")
    plt.show() 
    plt.clf() 
...
if __name__ == "__main__": 
    ...
    if("w_.oracleGeneral.bin.zst" in ap.tracepath): 
        ts_lists_for_all_ws,hr_lists_for_all_ws = [],[]
        for i in range(81,90):
            tracepath = ap.tracepath.replace("_",str(i)) 
            ts_lists_for_algos,hr_lists_for_algos,labels = [],[],[] 
            name = "Cloudphysics_w" + str(i); 
            
            algos = ALGOS if (ap.algo=="") else [ap.algo]
            
            for algo in algos:
                print("running",tracepath,"with", algo) 
                ts_list,hr_list = run(tracepath,ap.max_reqs,algo=algo,cache_size = ap.cache_size) 
                ts_lists_for_algos.append(ts_list) 
                hr_lists_for_algos.append(hr_list) 
                
                label = algo + "_" + "w" + str(i);
                labels.append(label)

            plot_hr_time(ts_lists_for_algos,hr_lists_for_algos,labels,name=name)
    else:
        print("running ",ap.tracepath)
        ts_list,hr_list = run(ap.tracepath,ap.max_reqs,algo=algo,cache_size = ap.cache_size) 
        plot_hr_time([ts_list],[hr_list],[ap.label])
